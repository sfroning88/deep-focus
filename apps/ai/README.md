...
Run installs from **`apps/ai`** so the editable path resolves. Use **Python 3.13** (same as **`python:3.13-slim`** in the Dockerfile); on macOS/Homebrew this is typically `python3`.
...
The **`Dockerfile`** expects the **repository root** as build context. **`WORKDIR`** is **`/repo/apps/ai`** so **`../../packages/python`** matches **`/repo/packages/python`**. Render **`buildFilter`** paths include **`packages/python/**`\*\* so shared-package changes trigger rebuilds.
